...
# ── Subcommand: claude-commands ────────────────────────────────────────────

# Skills shipped by the v5 framework. If a new skill lands in a future
# release, add its name here. The helper only touches files in this list,
# so project-specific commands (e.g. `dev.md`, `recruitment-intelligence.md`)
# are never overwritten.
V5_SKILL_NAMES = [
    "spec", "refine", "ux", "build", "validate", "review", "ship",
    "next", "status", "resume", "scan", "help", "migrate",
]

# Marker that identifies a file as a v5 framework wrapper. When a user has
# customised their wrapper, they can remove this marker to opt out of
# future auto-refreshes.
V5_WRAPPER_MARKER = "<!-- sdd-v5-wrapper: managed by migrate-v4-to-v5.sh -->"
...
def _render_wrapper(name: str, description: str) -> str:
    """Render a thin v5 command wrapper. Intentionally generic — projects
    should customise afterwards if they want to reference specific tooling
    (Biome, SonarQube, Git Flow, etc.)."""
    return f"""---
name: {name}
...
You are executing the `/{name}` skill from the SDD framework v5.

**Argument**: $ARGUMENTS (see `framework/skills/{name}/SKILL.md` for the exact contract).

**Step 1 — Load the skill definition**
Read `framework/skills/{name}/SKILL.md` and follow it exactly.

**Step 2 — Delegate to the orchestrator**
For gates and pipeline steps, the executable orchestrator is the single source
of truth in v5:

```bash
python3 framework/scripts/orchestrator.py --mode {name} $ARGUMENTS
```

(if `{name}` is not a direct orchestrator mode, the skill file documents the
right entrypoint — the Python report script for `next`, the generator script
for `ship`, etc.)

**Step 3 — Load context**
- `memory/{{project}}.md` (project state)
- `memory/LESSONS.md` (known pitfalls)
- `specs/feature-tracker.yaml` (story lifecycle)
- `specs/<project>.yaml` (root spec — drives gate selection via `spec.type`)
- `_work/stacks/registry.yaml` + the active stack profiles

**Step 4 — Respect the v5 contract**
- G1–G14 are adaptive per `spec.type`. Never skip a gate silently.
- Agents loaded by this skill live in `framework/agents/*.md` (see
  `_docs/PIPELINE.md` for the v5 roster of 18 agents).
- The cycle counter trips at 3. After that, the story is `escalated` and
  only `/resume <story-id> "<reason>"` unlocks it — reason is mandatory.

**Project-specific customisations**
This wrapper is generated by the v4→v5 migration script and is intentionally
generic. Customise freely — references to project tooling (Biome, SonarQube,
Git Flow base branch, etc.) go here. To opt out of future auto-refreshes,
remove the `{V5_WRAPPER_MARKER}` line above.
"""


def cmd_claude_commands(args: argparse.Namespace) -> int:
    """Generate / refresh `.claude/commands/*.md` wrappers for every v5 skill.

    Write policy:
      - If the file does NOT exist          → write a fresh wrapper.
      - If the file exists AND has the v5   → regenerate (safe: user opted in).
        marker (V5_WRAPPER_MARKER)
      - If the file exists AND looks v4     → regenerate (we're migrating).
        ("SDD framework v2" / "v4" marker)
      - Otherwise                            → leave alone (project customised
        it and opted out of auto-refresh).
    """
    project = Path(args.project).resolve()
    ...
    if not skills_dir.is_dir():
        print(f"[claude-commands] skills/ missing at {skills_dir} — skipping")
        return 0
    ...
    for name in V5_SKILL_NAMES:
        ...
        # Fall back to the skill name + a generic description if the
        # frontmatter is incomplete (shouldn't happen for shipped skills).
        desc = fm.get("description") or f"Execute the /{name} v5 skill"
        rendered = _render_wrapper(name, desc)

        target = commands_dir / f"{name}.md"
        if target.exists():
            existing = target.read_text(encoding="utf-8")
            is_managed = V5_WRAPPER_MARKER in existing
            is_v4_stale = (
                "SDD framework v2" in existing
                or "SDD framework v3" in existing
                or "SDD framework v4" in existing
            )
            if not (is_managed or is_v4_stale):
                # User customised it AND removed the marker → opt-out.
    ...
    print(
        f"[claude-commands] written={written} "
        f"skipped_user_owned={skipped_user_owned} "
        f"skipped_missing_skill={skipped_missing_skill}"
    )
    return 0
